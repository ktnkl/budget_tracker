...
function getBudget(state) {
    ...
    for (item of state.categories){
        if (item.categoryId == 1) for(let v = 0; v < item.value.length; v++)budget += Number(item.value[v]);
        else if (item.categoryId == 2) for(let v = 0; v < item.value.length; v++)budget -= Number(item.value[v]);
    }
    state.budget = budget;
    saveState();
    renderBudget(state);
...
function addCategory(categoryId) {
    ...
    const inputId = `add-${(0, _utilsJs.cat)(categoryId)}-category-name`;
...
function addCategoryValue(categoryId) {
    const inputId = `add-${(0, _utilsJs.cat)(categoryId)}-value`;
...
function renderPage(categoryId) {
    ...
    $mainWrapper.insertAdjacentHTML("beforeend", (0, _templatesJs.tab)(categoryType));
...
function renderRadio(categoryId) {
    const radioName = `${(0, _utilsJs.cat)(categoryId)}`;
    const inputId = `${(0, _utilsJs.cat)(categoryId)}-radios`;
    const $radios = document.getElementById(inputId);
    $radios.innerHTML = "";
    for(let i = 0; i < state.categories.length; i++)if (state.categories[i].categoryId == categoryId) {
        let { id, categoryName } = state.categories[i];
        $radios.insertAdjacentHTML("beforeend", (0, _templatesJs.radio)(id, categoryName, radioName));
    }
}
function renderCategory(categoryId) {
    const radioName = `${(0, _utilsJs.cat)(categoryId)}`;
    const categoriesListId = `categories-${(0, _utilsJs.cat)(categoryId)}-list`;
    ...
    for(let i = 0; i < state.categories.length; i++)if (state.categories[i].categoryId == categoryId) {
        ...
        $categoriesList.insertAdjacentHTML("beforeend", (0, _templatesJs.category)(id, categoryName, valueSum));
    }
}
...
function tab(categoryType) {
    ...
    <article id="${categoryType}" class="${categoryType}">
      ...
      <button class="${categoryType}-form__add" id="add-${categoryType}" >\u{414}\u{43E}\u{431}\u{430}\u{432}\u{438}\u{442}\u{44C} \u{437}\u{43D}\u{430}\u{447}\u{435}\u{43D}\u{438}\u{435}</button>
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["1D0jQ","8ZNvh"], "8ZNvh", "parcelRequire6e21")
